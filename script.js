...
const API_URL_CANDIDATES = [
  /* Deployment aktif 26 Agu 2026 ΓÇö v3.0.7-final.1 + AUTH Phase-1 DORMANT (YPTT_AUTH_USERS=0) */
  'https://script.google.com/macros/s/AKfycbz7_2Ht3NAxcSjgalOV54samRX-iAGdCysJm2ZzGsdd9R8HsiMOZo3rfOGC4mwjB8-K/exec',
...
/** Token otorisasi (VIEWER/OPERATOR/ADMIN) ΓÇö diset via panel Akses Operator.
...
async function resolveApiBaseUrl(force) {
  ...
  console.warn('[YPTT] Semua kandidat backend gagal ΓÇö memakai', API_BASE_URL);
  return API_BASE_URL;
}
...
const HEADER_ICONS = {
  'No': '≡ƒöó', 'WID': '≡ƒôï', 'Site ID Impl': '≡ƒåö', 'Site ID': '≡ƒåö',
  'Site Name Impl': '≡ƒôì', 'Site Name': '≡ƒôì', 'Band': '≡ƒô╢',
  'ZTE ZONE': '≡ƒîì', 'Zona': '≡ƒîì', 'Branch': '≡ƒîì',
  'SM Status': '≡ƒôè', 'SM ATP': '≡ƒöä', 'ATP Passed': 'Γ£à',
  'HI Progress': 'ΓÅ▒∩╕Å', 'HI Done': 'Γ£ö∩╕Å', 'Connected Date': '≡ƒöù',
  'FI Ineom': '≡ƒº╛', 'MOS': '≡ƒ¢░∩╕Å', 'Kategori': '≡ƒôè', 'Bulan': '≡ƒôà',
  'Jumlah': '≡ƒöó', 'Plan': '≡ƒÄ»', 'Ach': 'Γ£à', 'Persen': 'ΓÅ▒∩╕Å',
  'Milestone': '≡ƒÅü', 'PO Year': '≡ƒùô∩╕Å', 'Remarks': '≡ƒô¥', 'Total Site': '#'
};
...
let state = {
  ...
  pendingWrite: null,   // {retry:'form'|'delete', sheetKey, rowIndex} ΓÇö dilanjutkan otomatis setelah login sukses
  dashPg: {             // pagination Ringkasan Sulawesi ΓÇö TERPISAH dari state.dashboard
...
function updateAuthStatusBadge(info) {
  ...
  if (!statusEl) return;
  if (info && info.role) {
    statusEl.textContent = 'ΓùÅ ' + info.role + (info.canWriteRaw ? ' (boleh edit RAW)' : ' (read-only)');
    statusEl.className = 'badge ' + (info.canWriteRaw ? 'badge-success' : '');
  } else {
    statusEl.textContent = 'ΓùÅ Anonymous';
...
function renderAuthPanel() {
  ...
  if (authenticated) {
    box.innerHTML =
      ...
      '<span class="badge">' + esc(r) + (r !== 'VIEWER' ? ' ΓÇö RAW write aktif' : ' ΓÇö read-only') + '</span></div>' +
    ...
    box.innerHTML =
      ...
      '<span class="badge">Anonymous ΓÇö READ only</span></div>' +
 ...
 * - pivot-* diblokir frontend (Pivot Lock) ΓÇö backend tetap authority.
 */
 ...
 * Tidak ada retry otomatis berulang ΓÇö lanjut hanya SEKALI setelah login sukses.
 */
...
function mapAuthError(err) {
  ...
  return null; // bukan error auth/network ΓÇö biarkan pesan asli
}

/* ==================== Data Version (spec V2 ┬º20-21) ====================
...
/* ==================== DATA CONTRACT LAYER ====================
 * Normalizer + validator tunggal antara API dan renderer (spec fix ┬º2-┬º8).
...
function normalizeTableData(data, label) {
  ...
  // Jika headers kosong tapi rows ada ΓåÆ deteksi dari baris pertama (legacy)
...
function pickDerived(engTbl, legTbl) {
  // Prioritas WAJIB (spec fix ┬º3/┬º8): ENGINE > manual legacy > empty-state.
  ...
  return {
    ...
    emptyMessage: (engTbl && engTbl.label)
      ? 'ENGINE belum dibuat ΓÇö jalankan Engine dari Admin Tools'
      : 'Data belum tersedia'
  };
}
...
function renderDashboardStage3(skipPageReset = false) {
  ...
  if (d) {
    ...
    // --- Pagination untuk Ringkasan Sulawesi (state.dashPg ΓÇö aman dari payload overwrite) ---
...
/**
 * Status semantik frontend ΓÇö MIRROR dari classifyStatus_/parseDateLoose_ Code.gs.
...
function renderHeaderStats() {
  const kpi = state.kpi || {};
  const stats = [
    { icon: '≡ƒ¢░∩╕Å', value: fmt(kpi.total_mos), label: 'Total MOS' },
    { icon: 'Γ£à', value: fmt(kpi.total_hi_done), label: 'HI Done' },
    { icon: '≡ƒöù', value: fmt(kpi.total_connected), label: 'Connected' },
    { icon: 'ΓÜí', value: fmt(kpi.total_sm_atp), label: 'SM ATP' },
    { icon: '≡ƒôï', value: fmt(kpi.total_fi_ineom), label: 'FI INEOM' },
    { icon: 'ΓÜá∩╕Å', value: fmt(countIssues()), label: 'Issue / Blocking', danger: true }
  ];
...
function renderLatestTable() {
  ...
  const tbody = '<tbody>' + top.map(item => {
    ...
    return '<tr class="clickable" onclick="showDetailModal(\'' + item.key + '\', ' + r.rowIndex + ')">' +
      ...
      '<td>' + esc(fmt(r['MOS'])) + '</td>' +
      '<td>' + esc(fmt(isDoneVal(r['HI Done']) ? 'Γ£ö' : '-')) + '</td>' +
      '<td>' + esc(fmt(isDoneVal(r['Connected Date']) ? 'Γ£ö' : '-')) + '</td>' +
      '<td>' + (badge || esc(fmt(r['SM Status']))) + '</td>' +
      '<td><button class="btn btn-primary btn-sm" onclick="event.stopPropagation();showDetailModal(\'' +
        item.key + '\', ' + r.rowIndex + ')">≡ƒæü Detail</button></td>' +
    '</tr>';
  }).join('') + '</tbody>';
...
async function loadSheet(sheetKey, force = false) {
  ...
  try {
    let j;
    if (c.api === 'pivot') {
      // Resolver ENGINE > legacy (spec fix ┬º4/┬º9)
...
function renderKPI() {
  ...
  const cards = [
    { icon: '≡ƒôí', label: 'Total Site', value: fmt(kpi.total_site), cls: '' },
    { icon: '≡ƒ¢░∩╕Å', label: 'Total MOS', value: fmt(kpi.total_mos), pct: pctOf(kpi.total_mos, total), cls: 'kpi-mos' },
    { icon: 'Γ£à', label: 'Total HI Done', value: fmt(kpi.total_hi_done), pct: pctOf(kpi.total_hi_done, total), cls: 'kpi-hi' },
    { icon: '≡ƒöù', label: 'Connected', value: fmt(kpi.total_connected), pct: pctOf(kpi.total_connected, total), cls: 'kpi-connect' },
    { icon: 'ΓÜí', label: 'SM ATP', value: fmt(kpi.total_sm_atp), pct: pctOf(kpi.total_sm_atp, total), cls: 'kpi-atp' },
    { icon: '≡ƒº╛', label: 'FI INEOM', value: fmt(kpi.total_fi_ineom), pct: pctOf(kpi.total_fi_ineom, total), cls: 'kpi-ineom' }
  ];
...
function renderMiniTable(divId, sheetData) {
  ...
  // Kontrak: normalisasi + assert ΓÇö renderer tidak pernah melihat bentuk mentah
...
// Event filter & search ΓÇö reset ke halaman 1 saat berubah
...
function saveForm() {
  ...
  apiWrite(action, payload)
    ...
    .catch((err) => {
      // Form TIDAK direset ΓÇö user dapat memperbaiki token lalu Save lagi.
...
function renderReconciliation(data) {
  ...
  data.sections.forEach((sec, si) => {
    ...
    html += '<div class="table-scroll"><table class="data-table compact" id="reconTable-' + si + '">' +
      ...
      '</tr></thead><tbody>';
    sec.rows.forEach(r => {
      const deltaTxt = r.delta === null ? 'ΓÇö' : String(r.delta);
      ...
      if (sec.metricMeta && sec.metricMeta[r.key[0]] &&
        ...
        snapTag = ' <span class="badge" title="' +
          esc(sec.metricMeta[r.key[0]].note || 'Snapshot saat ini') + '">ΓÅ▒snapshot</span>';
      }
      html += '<tr' + (bad ? ' class="clickable"' : '') +
        ...
        '<td class="num">' + esc(r.manual === null ? 'ΓÇö' : r.manual) + '</td>' +
        '<td class="num">' + esc(r.engine === null ? 'ΓÇö' : r.engine) + '</td>' +
...
/** Klik baris delta ΓåÆ tampilkan/sembunyikan daftar WID penyusun metrik tsb */
async function reconDrill(tr) {
  ...
  try {
    ...
    const list = d.rows.slice(0, 50).map(r =>
      esc(r.wid) + (r.siteName ? ' ΓÇö ' + esc(truncate(r.siteName, 34)) : '') +
    ...
    td.innerHTML =
      ...
      (list || '<i>Tidak ada baris ΓÇö kemungkinan selisih berasal dari definisi/filter</i>') +
      '</div>' +
      '<div style="margin-top:6px;font-size:.72rem;opacity:.75;">Bandingkan dengan catatan manual. ' +
      'Jika WID engine tampak benar ΓåÆ cache/manual yang usang. Jika ada tanggal invalid ΓåÆ perbaiki RAW.</div>';
  } catch (err) {
...
async function runEngineSync() {
  ...
  try {
    if (!confirm('Jalankan calculation engine?\n\nHasil ditulis ke sheet ENGINE_* ΓÇö sheet manual & RAW tidak disentuh.')) return;
